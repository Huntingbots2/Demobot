...
from AsunaRobot import DRAGONS as DEVS
from AsunaRobot import OWNER_ID
from AsunaRobot import telethn as  Asuna
from AsunaRobot.events import  Asunainline
from AsunaRobot.events import register as  Asunabot
from AsunaRobot.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await  Asuna(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await  Asuna(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@ Asunabot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
